#http://www.codeskulptor.org/#user30_iqwBgdQ9mJPAWzO.py
...
def mouseclick(pos):
    ...
    for x in a:
        if (pos[0]>x and pos[0]<x+30 and pos[1]>=10 and pos[1]<=80):
            ...
            elif (state==1 and (x!=x1)):
                state=2
                x2=x
                turn+=1
                label.set_text("Turns = "+str(turn))
                print "state "+str(state)
            elif ((state==3) and (x!=x1) and (x!=x2)):
                x2=x
                state=2
                turn+=1
                label.set_text("Turns = "+str(turn))
